...
public class CmdLineWrapper extends CloudDataProcessorService
{
    ...
    public void execute() throws Exception
    {
        ...
        switch (Installer.getOsName()) {
            case "windows":
                ...
                pb.command("cmd.exe", "/C", mainScript.toString());
                break;
            case "linux":
            case "macos":
    ...
    throws XmlStorageException
    {
        ...
        for (Object name : properties.getNames()) {
            ...
            } else if (propValue instanceof XmlDateDataObject) {
                processEnvironment.put("PROPS__" + _toSimpleVariableName(name.toString()), format.format(((XmlDateDataObject) property).dateValue()));
            } else if (propValue instanceof XmlStorableDataObject) {
                if (propValue.getValue() instanceof DocumentRecord) {
                    DocumentRecord wrapper = (DocumentRecord) propValue.getValue();
                    ...
                    processEnvironment.put("PROPS__" + _toSimpleVariableName(name.toString()), downloadFile.getName());
                } else {
                    throw new UnsupportedOperationException("Property conversion for type: " + propValue.getValue().getClass().getSimpleName() + " has not been implemented.");
                }
            }
        } else {
